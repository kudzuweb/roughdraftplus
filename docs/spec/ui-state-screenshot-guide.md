...
| Editor | Unrendered block deletion refused | Open the unrendered block fixture in rich mode, choose Editing, click a placeholder, then press Backspace, Delete or any character | `unrendered-block-placeholder`, `unrendered-block-deletion-refused` | A second description line inside the same alert reads `A keystroke will not delete this <block type>. Switch to code view to remove its Markdown.` The block stays in the document and on disk. The note appears only in Editing mode and clears as soon as the selection moves; Suggesting mode never reaches the guard and shows no note. |
