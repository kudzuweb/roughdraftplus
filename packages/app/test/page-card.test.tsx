...
describe("PageCard editor integration", () => {
  ...
  it("holds an approved agent reply until pending approvals are applied, then drops only that reply in one save", async () => {
    const rendered = await renderPageCard({
      page: {
        id: "doc-approve-reply-1",
        title: "Doc Approve Reply 1",
        content:
    ...
    expect(
      queryByTestId(rendered.container, "comment-banner-a2-approval-pending"),
    ).not.toBeNull();
    ...
    const savedMarkdown = rendered.onSave.mock.calls[0]?.[1];
    expect(savedMarkdown).toContain("{==alpha==}");
    expect(savedMarkdown).toContain("Root comment");
