...
| 144 | It polls in 240 s slices with a cursor, and adds a bounded retry on transient fetch failures (5 consecutive, 2 s apart, logged) with tests for the crash, the slice contract, cursor resumption, and `--timeout` expiry. | Best upstream design. The fork already has the slicing; the transient retry and its test are the harvest, filed under #11 above. |
