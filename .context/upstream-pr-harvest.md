...
| 135 | Preserve single tilde text in markdown (peterhartree, 2026-07-08) | Skip: no open fork issue. The bug is present in the fork and a save rewrites `~57%` to `~~57%` (section 5); the fix, a custom marked `del` tokenizer that requires a double tilde, is small and sound and is the material for the ticket filed from section 5. |
