...
- **They implement a different boundary model from the live routes, though not a smaller reach.** Every live file route takes a `projectPath` and keeps relative paths inside it through `ensureProjectPath` (`packages/server/src/index.ts:216-229`), but `projectPath` itself is whatever the caller sends, checked only for being an existing directory (`index.ts:488-508`), so the live routes reach any file on the host too (probe L1 read `/private/etc/hosts` by naming `/private/etc` as the project). The dormant listing routes drop even that shape: they accept any absolute path with no project at all (`index.ts:1127-1131`, `1141-1145`), and `/api/project/create` runs `mkdir -p` on any absolute path (`index.ts:1204-1205`). ADR 0001 records that the server "resolves that file within local-file boundaries" and ADR 0004 that boundary checks "belong in the core server path"; neither model satisfies that today.
- **They sit outside the token guard, and so does every live file route.** The bearer-token check runs only on the four `/api/remote-document` routes (`index.ts:917`, `972`, `985`, `1046`). With a token configured and no `Authorization` header, `/api/fs/list` answered 200 (probe P16), `/api/files` served `/private/etc/hosts` (probe L1), `GET /api/markdown-file` read a file and `PUT /api/markdown-file` rewrote it on disk (probes L2, L3), and `/api/review-index` answered 200 (probe L4), while `/api/remote-document/:id` answered 401 (probe P17). The bind guard's error text names only the remote-document endpoints as the exposure (`index.ts:1278-1287`), so on a non-loopback bind anyone on that interface can read any file and read or write any `.md` file on the host without the token. Removing the five dormant routes narrows that surface without closing it. The follow-on security ticket, #47, must therefore cover every file-touching route on a non-loopback bind (require the token on each, or refuse to bind non-loopback until they are guarded), not only the dormant ones. The loopback default is protected by the browser's same-origin policy, since the server sends no CORS headers (probe L5 got no `Access-Control-Allow-Origin` for a foreign `Origin`), so the urgency is the Tailscale deployment the brainstorm in `docs/brainstorms/remote-document-mode-requirements.md` describes.
...
- **They model the wrong thing for the coming work.** #15 and #16 need per-open-document state: which file is open in which tab, which session opened it, when it was last saved, which threads are open. The dormant surface models directories to browse. The server already holds the first of those: `openRequestClients` (`index.ts:407`, registered at `851-870`) records, for every connected tab, the `?path=` it loaded, and the open-request handler searches it by exact path to decide whether an existing window is reused (`index.ts:897-899`), which is the duplicate-window mechanism backlog item 8 describes. The app re-subscribes with the new path whenever it navigates (`packages/app/src/App.tsx:1564-1568`), so that registry is a live record of what each tab has open. It carries no session label or timestamps and forgets a tab on disconnect (`index.ts:875-878`), so it is not yet what #15 and #16 need, but it is where that state lives; the `stateless: true` flag on `/api/status` (`index.ts:840`) is about project state, not this. The remote-session map (`index.ts:88-96`, `409`) is the other per-document state on the server. Nothing in the dormant surface touches either.
...
- The session label has no carrier today. The open-request payload is `path` and `url` only (`index.ts:83-86`, `881-891`), the SSE event forwards `path`, `url` and `instanceId` (`index.ts:906-911`), and the target URL carries only `path` (`cli.ts:1145`). The label needs a new field on both, plus a field on `BackendInfo` (`packages/app/src/storage.ts:52-60`) to reach the header. That is new state, and its home is the open-request registry the server already keeps per tab (`openRequestClients`, `index.ts:407`, `851-870`), extended as described under #16, not the dormant surface and not a second path-keyed map beside the registry.

The switch warning in #15 hangs on the same registry: it already knows which path each tab has open (the tab subscribes with its path at `App.tsx:1564-1568`), and the open-request handler already compares the incoming path against it (`index.ts:897-899`). Today a mismatch means "no matching window, open a new one"; #15 turns it into a warning or a new tab, and backlog item 8's window-reuse fix changes the same comparison, so both land in one structure. (`App.tsx:1607-1677` is the load path that runs after a navigation.) Issue #15 and backlog item 8 cite that match as `index.ts:869-871`; at 04c44ae those lines are the keep-alive setup, and the match is at `897-899`.
...
The record #16 needs is a per-document entry: absolute path, session label, opened-at, last-saved-at, and the server that owns it. Build it by extending the open-request registry the server already keeps (`openRequestClients`, `index.ts:407`, `851-870`, matched at `897-899`), which already holds the absolute path per connected tab, rather than adding a second path-keyed map beside it: give its entries the label, the timestamps and whatever must outlive a tab's disconnect (`index.ts:875-878` drops an entry today). Populate the new fields from the open path (the CLI's open request at `index.ts:881-914` and the app's first load through `GET /api/markdown-file` at `index.ts:569-588`) and from the two save routes. Keep it in memory only; the state file stays a process record. That is an explicit extension of ADR 0004's 2026-04-30 clarification, which admits transient in-memory server state for remote-document sessions only, to local documents; the same reasoning applies (the state file does not become a document model), and the ADR should say so when #15 or #16 lands. A multi-document workspace beyond that still needs the separate decision ADR 0001's consequences clause calls for; this record is the single-file-compatible seed of that track, not the decision. Expose it through `/api/status` or a new read route keyed by absolute path, so `status` can name the document and `status <path>` can say "not open" for any other path. The CLI change is then a positional on `status` that reads that route.
...
- Probe: 200 inside the project (P12), 404 for `../` traversal (P13) and for an absolute path outside (P14). The boundary holds only relative to the caller-supplied `projectPath`: naming `/private/etc` as the project served `hosts` (probe L1), and no token is checked.
- The issue listed it with the dormant routes; it is not one, and the verdict excludes it. Keep it and `/api/assets` (`index.ts:1230-1256`, called from `api-backend.ts:186-209`); guarding both on a non-loopback bind is part of #47.
...
- Keep `/api/files` and `/api/assets`, `ensureProjectPath`, and everything under `/api/markdown-file`, `/api/review-index`, `/api/review-events`, `/api/open-request` and `/api/remote-document`. Keeping them keeps the non-loopback exposure described under the verdict: every one of the file-touching routes among them accepts a caller-supplied `projectPath` (`index.ts:488-508`) and none checks the token. Issue #47 covers guarding all of them on a non-loopback bind together with the removal above; the removal alone is not a security fix.
...
- Runtime behavior came from an in-process server built with `createApp({ port: 4640, remoteDocumentToken: "probe-secret" })` listening on 127.0.0.1, with eighteen requests numbered P1 to P18 above and a second run of five, L1 to L5, against the live routes: `/api/files` with `projectPath=/private/etc` and `path=hosts`, `GET` and `PUT /api/markdown-file` on a scratch `.md` outside the repo, `/api/review-index` on the same file, and `/api/status` with a foreign `Origin` header, all sent without an `Authorization` header while the token was configured. The server was closed and the created directory deleted at the end of each run.
