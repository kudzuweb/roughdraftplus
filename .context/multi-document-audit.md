...
The switch warning in #15 hangs on the same registry: it already knows which path each tab has open (the tab subscribes with its path at `App.tsx:1564-1568`), and the open-request handler already compares the incoming path against it (`index.ts:897-899`). Today a mismatch means "no matching window, open a new one"; #15 turns it into a warning or a new tab, and backlog item 8's window-reuse fix changes the same comparison, so both land in one structure. (`App.tsx:1607-1677` is the load path that runs after a navigation.)
